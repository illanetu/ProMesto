Ты — ведущий fullstack-разработчик. Работаем в существующем проекте ProMesto (Next.js App Router + TypeScript + Prisma + Neon/Postgres + Auth). Нужно добавить лайки к ПУБЛИЧНЫМ местам.

ЦЕЛЬ
- Пользователь может поставить лайк публичному месту.
- Ограничение: 1 пользователь = 1 лайк на место (повторное нажатие снимает лайк — toggle).
...
- Лайкать можно только public mesto.
...
- Ошибки: если не авторизован — показать понятное сообщение/редирект на вход; если место приватный/не найден — 404/ошибка; если БД недоступна — “Попробуйте позже”.
...
2) Серверная логика:
   ...
   - Проверь: место существует и isPublic=true.
   - Toggle:
...
3) Вывод данных:
   - В выборке публичных мест добавь:
     - likesCount (через _count.likes)
     - признак likedByMe для текущего пользователя (через relation filter или отдельный запрос по mestoIds)
   - Реализуй сортировки:
...
4) UI:
   - Компонент LikeButton:
     props: mestoId, initialLiked, initialCount
     состояние: liked/count/loading
     по клику -> fetch POST /api/mestos/{id}/like
     обновить UI по ответу, обработать ошибки.
   - Вставь кнопку в карточку промта в “Публичные места”.
