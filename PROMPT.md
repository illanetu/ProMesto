Ты — ведущий fullstack-разработчик. Работаем в существующем проекте ProMesto (Next.js App Router + TypeScript + Prisma + Neon/Postgres + Auth). Нужно добавить лайки к ПУБЛИЧНЫМ промтам.
...
- Защита от накрутки на уровне БД: уникальный индекс (userId, mestoId).
...
1) Схема БД:
   - Добавь модель Like (или PromptLike) в prisma.schema:
     fields: id (cuid), userId, mestoId, createdAt
     relations: Like->User, Like->mesto
     @@unique([userId, mestoId])
     @@index([mestoId])
   - Обнови модель mestos: likes relation
   - Сделай миграцию Prisma.

2) Серверная логика:
   - Создай route handler: POST /api/mestos/[id]/like
